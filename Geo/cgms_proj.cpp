...
NormGeosProj::NormGeosProj(float lon,
                           float coff,
                           long int cfac,
                           float loff,
...
void NormGeosProj::ScaleFunc(double x, double y, float &col, float &row)
{
...
void NormGeosProj::ScaleFuncInv(float col, float row, double &x, double &y)
{
    double val;
    val = pow(2, -16);
...
void NormGeosProj::GeoToImage(float lon, float lat, float &col, float &row)
{
    ...
    // 计算地心经纬度
    double phiR;
    phiR = atan(pow(r_pol / r_eq, 2)) * tan(latR);

    double val, re;
    val = (pow(r_eq, 2) - pow(r_pol, 2)) / pow(r_eq, 2);
    re = r_pol / sqrt(1 - val * pow(cos(phiR), 2));
...
void NormGeosProj::ImageToGeo(float col, float row, float &lon, float &lat)
{
    double x, y;
    ...
    double val1, val2;
    val1 = h*cos(x)*cos(y);
    val2 = pow(cos(y), 2) + pow(r_eq / r_pol * sin(y), 2);

    double sd_val, sn_val;
    double s1, s2, s3, sxy;
    sd_val = sqrt(pow(val1, 2) - val2*(pow(h, 2) - pow(r_eq, 2)));
    ...
    double lonR, latR;
    lonR = atan(s2/ s1);
    latR = atan(pow(r_eq, 2) / pow(r_pol, 2) * s3 / sxy);
...
// int main()
// {
//     NormGeosProj gs1(140.7, 11000.5, 81865099, 11000.5, 81865099);
